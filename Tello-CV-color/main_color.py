...
def main():
	...
	try:
		while True:
			...
			bgr_image = cv2.resize(image, dsize=(960,720) )	# 画像サイズを半分に変更

			hsv_image = cv2.cvtColor(bgr_image,cv2.COLOR_BGR2HSV) #BGR画像からHSV画像へと変換　検知しやすいから

			# (X)ウィンドウに表示
			# BGRとHSVを２つウィンドウで表示して、比較
